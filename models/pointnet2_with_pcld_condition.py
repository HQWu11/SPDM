...
class PointNet2CloudCondition(PointNet2SemSegSSG):
    ...
    def _build_model(self):
        ...
        if self.include_local_feature:
            ...
            npoint_condition = condition_arch['npoint']
            radius_condition = condition_arch['radius']
            nsample_condition = condition_arch['nsample']
            feature_dim_condition = condition_arch['feature_dim']
            mlp_depth_condition = condition_arch['mlp_depth']
            ...
            encoder_feature_map_dim = mapper_arch['encoder_feature_map_dim']
        ...
        self.att_c_stack = nn.ModuleList([
	    AttentionFusion(
		dim=kv,
		depth=1, 
		latent_dim=q,
        ...
        self.att_noise_stack = nn.ModuleList([
	    AttentionFusion(
		dim=q,
		depth=1,
		latent_dim=kv,
        ...
        npoint = arch['npoint']
        radius = arch['radius']
        nsample = arch['nsample']
        feature_dim = arch['feature_dim']
        mlp_depth = arch['mlp_depth']
        ...
        if self.include_local_feature:
            ...
            decoder_feature_dim_condition = condition_arch['decoder_feature_dim']
            decoder_mlp_depth_condition = condition_arch['decoder_mlp_depth']
            ...
            decoder_feature_map_dim = mapper_arch['decoder_feature_map_dim']


        include_grouper = arch.get('include_grouper', False)
        use_knn_FP =  arch.get('use_knn_FP', False)
        K = arch.get('K', 3)
        decoder_feature_dim = arch['decoder_feature_dim']
        decoder_mlp_depth = arch['decoder_mlp_depth']
    ...
    def forward(
        ...
        cond_feat = l_cond_features[-1].permute(0, 2, 1)
        noisy_feat = l_features[-1].permute(0, 2, 1)

        for att in self.att_c_stack:
            cond_feat = att(noisy_feat, queries_encoder=cond_feat)

        for att in self.att_noise_stack:
            noisy_feat = att(cond_feat, queries_encoder=noisy_feat)

        l_cond_features[-1] = cond_feat.permute(0, 2, 1).contiguous()
        l_features[-1] = noisy_feat.permute(0, 2, 1).contiguous()
